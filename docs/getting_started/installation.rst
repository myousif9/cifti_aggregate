...
Install from github with pip::
    ...
    pip install -e git+https://github.com/myousif9/cifti_aggregate#egg=cifti_aggregate
...
Do a dry-run first (``-n``) and simply print (``-p``) what would be run::
    ...
    cifti_aggregate /path/to/bids/dir /path/to/output/dir participant -np

Run the app, using all cores::
    
    cifti_aggregate /path/to/bids/dir /path/to/output/dir participant --cores all
...
To generate a report, run::
    ...
    cifti_aggregate /path/to/bids/dir /path/to/output/dir participant --report
...
Here are some instructions to get your python environment set-up on graham to run cifti_aggregate:

#. Create a virtualenv and activate it::

      cd $SCRATCH
      module load python/3
      virtualenv venv_cifti_aggregate
      source venv_cifti_aggregate/bin/activate
...
Scaling up to even more subjects (uses group-components to bundle multiple subjects in each job), 1 32core job for N subjects (e.g. 10)::
    ...
    cifti_aggregate bids_dir out_dir participant  --profile cc-slurm --group-components subj=10
